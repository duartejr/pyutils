...
def idw(
    ...
    # Validation.
    if station_rows.size == 0:
        raise ValueError(
            "Must provide at least one station (received empty station_rows)"
        )
    if (
        station_rows.size != station_cols.size
        or station_rows.size != station_values.size
    ):
        raise ValueError(
            f"Mismatched array sizes: "
            f"station_rows {station_rows.size}, "
            f"station_cols {station_cols.size}, "
            f"station_values {station_values.size}"
        )
